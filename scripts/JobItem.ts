	...
	.then((item: any) => {
		...
		switch (item["JobFamily"]["Title"]) {
			case "Group Finance and Technical Specialisms":
				...
				$("#lower-bg-container").addClass("bg-group-finance");
				break;
			case "Operational Finance":
				...
				$("#lower-bg-container").addClass("bg-operational-finance");
				break;
			case "Finance Shared Service Centre (FSSC)":
				...
				$("#lower-bg-container").addClass("bg-fssc");
				break;
		}
	})
	.catch(() => {
		window.location.href = "/sites/hrcareerpathways/Pages/index.aspx";
	});
